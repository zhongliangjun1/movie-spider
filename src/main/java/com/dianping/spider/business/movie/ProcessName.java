package com.dianping.spider.business.movie;

/**
 * Created with IntelliJ IDEA.
 * Author: liangjun.zhong
 * Date: 14-1-15
 * Time: PM1:27
 * To change this template use File | Settings | File Templates.
 */
public class ProcessName {

    public final static String GET_CINEMA_PLAY_ITEM_ACCORD_DATE_PROCESS = "GET_CINEMA_PLAY_ITEM_ACCORD_DATE%s_PROCESS";
    //key of the result map
    public final static String MOVIE_GEWARA_BASIC_LIST_RESULT_KEY = "MOVIE_GEWARA_BASIC_LIST_RESULT_KEY";
    public final static String MOVIE_PLAY_ITEM_LIST_GEWARA_LIST_RESULT_KEY = "MOVIE_PLAY_ITEM_LIST_GEWARA_LIST_RESULT_KEY";


    public final static String GET_MOVIE_INFO_FROM_DOUBAN_PROCESS = "GET_MOVIE_%s_INFO_FROM_DOUBAN_PROCESS";

}
